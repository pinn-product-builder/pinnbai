/**
 * Layout do App (clientes genéricos)
 */

import React, { useState } from 'react';
import { NavLink, useLocation, Outlet, useNavigate } from 'react-router-dom';
import { 
  LayoutDashboard, Database, Layers, GitBranch, FileText, Users,
  Settings, ChevronLeft, ChevronRight, LogOut, Sun, Moon, Plus
} from 'lucide-react';
...
const navItems = [
  { path: SAAS_ROUTES.APP.DASHBOARDS, label: 'Dashboards', icon: LayoutDashboard },
  { path: SAAS_ROUTES.APP.DATA_SOURCES, label: 'Data Sources', icon: Database },
  { path: SAAS_ROUTES.APP.DATA_SETS, label: 'Data Sets', icon: Layers },
  { path: SAAS_ROUTES.APP.PIPELINES, label: 'Pipelines', icon: GitBranch },
  { path: SAAS_ROUTES.APP.TEMPLATES, label: 'Templates', icon: FileText },
  { path: SAAS_ROUTES.APP.USUARIOS, label: 'Usuários', icon: Users },
  { path: SAAS_ROUTES.APP.CONFIGURACOES, label: 'Configurações', icon: Settings },
];
...
export function AppLayout() {
  ...
  const { signOut, user, isAdmin } = useSaasAuth();
      ...
      <aside className={cn(
        ...
        <div className="h-16 flex items-center px-4 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            {!collapsed && <span className="font-semibold text-lg text-text-1">Pinn</span>}
          </div>
        </div>
        ...
        {isAdmin && (
          <div className="px-3 mt-4 pt-4 border-t border-border">
            <NavLink
              ...
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-text-2 hover:text-text-1 hover:bg-bg-2"
            >
      ...
      <div className={cn("flex-1 flex flex-col transition-all duration-300", collapsed ? "ml-16" : "ml-60")}>
        <header className="sticky top-0 z-30 h-16 border-b border-border bg-bg-0/80 backdrop-blur-xl flex items-center justify-between px-6">
          <div className="flex items-center gap-4">
            <h2 className="text-lg font-semibold text-text-1">Plataforma</h2>
          </div>
          <div className="flex items-center gap-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button size="sm" className="bg-pinn-orange-500 hover:bg-pinn-orange-600">
                  <Plus className="w-4 h-4 mr-2" />
                  Criar
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="bg-bg-1 border-border">
                <DropdownMenuItem onClick={() => navigate('/app/dashboards?new=true')}>
                  <LayoutDashboard className="w-4 h-4 mr-2" />
                  Novo Dashboard
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => navigate('/app/data-sources?new=true')}>
                  <Database className="w-4 h-4 mr-2" />
                  Novo Data Source
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => navigate('/app/data-sets?new=true')}>
